...
decisão, com motivo obrigatório que sobrevive na razão. Publicado e provado
no `staging-v2`; **PROD apenas lida, sem uma única escrita**. Ver `INV-004`.
...
| ARQ-007 | ~~**Divergência entre `api/schema.sql` e as migrations**~~ — **RESOLVIDA em 15/09/2026** (`b6d6ed7`). `migracao-pos-golive-1.sql` criava `maleta_item_variacoes`, `venda_item_correcoes` e 7 índices (não 5) que nunca voltaram ao schema | DDL derivado de uma cópia read-only de produção e comparado instrução a instrução com a migration — os três idênticos. `SO_NO_MIGRADO` em `src/migracao-variantes-test.mjs` foi a zero, estrutura mantida vazia para obrigar a próxima divergência a ser declarada por nome | fechada só em `schema.sql` e no teste; PROD continua congelada e não foi tocada |
...
| INV-004 | **Conciliação do inventário** — o fim que não existia. `POST /concluir {"contagemCompleta": true}`, motivos obrigatórios em `/aplicar`, revisão reorganizada pela decisão e progresso por categoria durante a contagem | **código feito e provado em 25/09/2026** (45 provas novas: 17 de domínio, 28 de frontend; release 18/18). Desenho em [INVENTARIO-4-4.md §14](../domains/INVENTARIO-4-4.md). **Publicado e provado no DEV:** migration aplicada em `marquesa-db-staging-v2` com contagens idênticas e razão fechando (1.485 = 1.485); 21 provas de QA visual no bundle publicado (desktop e 390px) e 28 provas ponta a ponta contra o Worker/D1 publicados — inclusive 633 códigos não bipados virando divergência candidata **sem mover a razão**. `marquesa-db-dev` **não** foi tocado: ele tem 29 tabelas contra 44 de PROD e nem `saidas_sem_faturamento` tem. PROD apenas lida |
